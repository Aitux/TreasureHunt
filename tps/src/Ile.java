...
public class Ile {
	...
	private static ArrayList<Personnage> Mort = new ArrayList<>();
	private Plateau p;
	...
	private String nomP(int ch){
		if(ch == 6) return "Explorateur";
		if(ch == 7) return "Explorateur";
		if(ch == 8) return "Voleur";
		if(ch == 9) return "Voleur";
...
/*		if(sandbox = true){
	...
	public void defaite(){
		int t1 = 0;
		int t2 = 0;
		for(Personnage p : Mort){
			if(p.isTeam()   == 1) t1++;
			if(p.isTeam() == 2) t2++;
		}
		if(t1 == 2){ JOptionPane.showMessageDialog(null, "L'équipe 1 a perdu"); System.exit(0);}
		if(t2 == 2){ JOptionPane.showMessageDialog(null, "L'équipe 2 a perdu"); System.exit(0);}
	}
	public void sandbox(){
		this.sandbox = true;
	}
